...
uint8_t userInputErrorFlag = 0;     // flag for when the user makes an error
uint8_t timeOut = 0;                // time out VAR
uint8_t indexInArray = 0;           // what index in the array are they
...
void verifySequence_tick(){
    ...
    case enable_vs_st:         //state number 2 for moore
        buttonHandler_enable();         // enable the button handler
        break;
    case wait_release_vs_st:         //state number 3 for moore
        timeOut++;                  // increment the timne out counter
        break;
    case region_vs_st:         //state number 4 for moore
        buttonHandler_disable();            // disable the button handler
        if(buttonHandler_getRegionNumber() != globals_getSequenceValue(indexInArray) ){ // if the area touch doesnt match the value the array has then user made an error
            userInputErrorFlag = TRUE;                                                  // raise flag
        }
        break;
    ...
    case init_vs_st:         //state number 1 for mealy
        if(verifyEnableFlag){
            timeOut = FALSE;                                // reset the VAR
            indexInArray = FALSE;                                // reset the VAR
            verifyIsCompleteFlag = FALSE;                                // reset the VAR
            timeOutErrorFlag = FALSE;                                // reset the VAR
            userInputErrorFlag = FALSE;                                // reset the VAR
    ...
    case wait_release_vs_st:         //state number 3 for mealy
        if(timeOut == TIME_OUT_NUM){
            verifyIsCompleteFlag = TRUE;                                // raise the flag that the Verify squence is done
            timeOutErrorFlag = TRUE;                                    // raise the flag that is took to long to press
            simonDisplay_eraseAllButtons();                             // erase all the buttons from the srceen
            verifySequenceCurrentState = finish_vs_st;
        }
        else if(display_isTouched()){
            timeOut = FALSE;                                            // if display is touch reset the timeout VAR
        }
        else if(buttonHandler_releaseDetected()){                       // if the screen was released then move to next state
            verifySequenceCurrentState = region_vs_st;
        }
        break;
    case region_vs_st:                                                   //state number 4 for mealy
        verifySequenceCurrentState = incrament_array_vs_st;                 //move to next state
        break;
    case incrament_array_vs_st:                                     // state number 5 for mealy
        if (indexInArray == globals_getSequenceIterationLength()){
            verifyIsCompleteFlag = TRUE;                            // set the complete flag to true
            ...
            indexInArray++;                                         // increment the nember in the array that the program is checking
            verifySequenceCurrentState = enable_vs_st;
        }
        break;
    case finish_vs_st:         //state number 6 for mealy
        if(!verifyEnableFlag){                                      // wait til the enable flag is lowered to exit the state machine
           verifySequenceCurrentState = init_vs_st;
        }
        break;
    }
