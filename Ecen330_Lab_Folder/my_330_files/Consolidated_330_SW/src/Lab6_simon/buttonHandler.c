...
void buttonHandler_tick(){                                          // Standard tick function.
    ...
    case button_delay_st:                                           // Mealy transition state action for state #4
        if (delayCounter == TRUE){                                  // after the little wait to
            display_clearOldTouchData();                            // clears the old data from the screen
            delayCounter = FALSE;                                       // reset the delayCounter for the next time through the SM
            simonDisplay_drawSquare(buttonHandler_getRegionNumber(), FALSE);// draw the square associated with the region that was touched
            region = buttonHandler_getRegionNumber();               // read the new touch data in from the screen to a VAR for later
            buttonHandlerCurrentState = button_touch_st;            // move to next state
        }
        break;                                                      // ends case
    case button_touch_st:                                           // Mealy transition state action for state #5
        ...
        break;                                                      // ends case
    case button_touch_release_st:                                   // Mealy transition state action for state #6
    ...
    case button_end_st:                                             // Mealy transition state action for state #7                                                //set the fifth state for moore
        if(!buttonEnableFlag){                                      // wait until the the flag is lowered
            buttonInitFlag = FALSE;                                 // set the initFlag off
